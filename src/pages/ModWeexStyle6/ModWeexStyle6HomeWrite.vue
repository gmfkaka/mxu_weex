     ...
     <div class="home-wrapper" :class="[this.isIphoneX() ? 'x-top':'']" :style="androidStylePaddingTop">
       ...
       <list class="list" show-scrollbar="false">
         <refresh @refresh="onrefresh" :display="refreshing ? 'show' : 'hide'" class="refresh">
          ...
          <cell>
              <list6 v-for="(item,index) in bookList"
                  :key="index"
                  :data="item.data[0].data.slice(0,2)"
                  :title="item.name"
                  :id="item.id"
                  :showMore="true"></list6>
          </cell>
...
export default {
  ...
  methods: {
    getData () {
      ...
      this.$httpRequest(this.api.my_write, param, 'get').then(res => {
        if (this.offset === 0) {
          this.bookList = res.slice(1, res.length)
        } else {
          if (res.length) {
            // 接口有问题 offset无效 暂时用这个判断
            if (res[1].name !== this.bookList[0].name) {
